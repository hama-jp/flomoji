...
interface UseWorkflowExecutionProps {
  ...
  nodeTypes: any;
  setNodes: (nodes: React.SetStateAction<WorkflowNode[]>) => void;
  setExecutor: (executor: Executor | null) => void;
  setExecutionState: (state: React.SetStateAction<ExecutionState>) => void;
  setExecutionResult: (result: ExecutionResult | null) => void;
  setDebugLog: (log: React.SetStateAction<LogEntry[]>) => void;
...
const useWorkflowExecution = ({
  ...
  const convertConnectionsFormat = useCallback(() => {
    ...
    return connections.map(edge => ({
      from: {
        nodeId: edge.source,
        portIndex: parseInt(edge.sourceHandle || '0') || 0
      },
      to: {
        nodeId: edge.target,
        portIndex: parseInt(edge.targetHandle || '0') || 0,
  ...
  const handleRunAll = useCallback(async () => {
    ...
    try {
      ...
      if (finalState.status === 'completed') {
        ...
        const completionLog: DebugLogEntry = {
      ...
      // 例外エラー時のログを追加
      const executionLog = nodeExecutionService.getExecutionLog();
      const exceptionLog: DebugLogEntry = {
        timestamp: new Date().toISOString(),
        level: 'error',
  ...
  const handleStepForward = useCallback(async () => {
    ...
    try {
      ...
      if (result.done) {
        ...
        const executionLog = nodeExecutionService.getExecutionLog();
        let newSelectedNode: WorkflowNode | null = null;

        setNodes((prevNodes: WorkflowNode[]) => {
          const newNodes = prevNodes.map((node: WorkflowNode) => {
            // Output ノードの結果を更新
        ...
        setExecutionState((prev: ExecutionState) => {
          const newExecutedIds = new Set<string>(prev.executedNodeIds);
          if (result.value.currentNodeId) {
            newExecutedIds.add(result.value.currentNodeId);
          }
