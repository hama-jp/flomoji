...
const DataView = () => {
  ...
  const [expandedWorkflows, setExpandedWorkflows] = useState(new Set()) // 展開されているワークフローID
  const [executionHistories, setExecutionHistories] = useState({}) // workflowId -> 実行履歴配列
  const [expandedRuns, setExpandedRuns] = useState(new Set()) // 展開されている実行ID
  const [runDetails, setRunDetails] = useState({}) // runId -> ノードログ配列
  ...
  const toggleWorkflowExpansion = async (workflowId) => {
                        ...
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-500">
                              {workflow.nodes?.length || 0} nodes, {workflow.edges?.length || workflow.connections?.length || 0} edges
                              {runs.length > 0 && (
