 ...
 */
export async function executeWebAPINode(data: any, inputs: NodeInputs): Promise<any> {
  const {
    url = '',
    method = 'GET',
    headers = {},
  ...
  if (dynamicPath && typeof dynamicPath === 'object') {
    Object.entries(dynamicPath).forEach(([key, value]: any) => {
  ...
  if (dynamicQuery && typeof dynamicQuery === 'object') {
    Object.entries(dynamicQuery).forEach(([key, value]: any) => {
    ...
    switch (bodyType) {
      ...
      case 'form':
        if (typeof dynamicBody === 'object') {
          ...
          Object.entries(dynamicBody).forEach(([key, value]: any) => {
  ...
  for (let attempt = 0; attempt <= retryCount; attempt++) {
    ...
    try {
      ...
      if (attempt === retryCount) {
        throw new Error(`WebAPIエラー: ${lastError instanceof Error ? lastError.message : String(lastError)}`);
      }
    }
  }
