...
import type { TimestampNodeData } from '../../types/nodeData';
import type { NodeExecuteFunction } from './types';
 ...
 */
const executeTimestampNode: NodeExecuteFunction = async (node: WorkflowNode, inputs: NodeInputs, context?: INodeExecutionContext): Promise<NodeOutput> => {
  const nodeData = node.data as TimestampNodeData;
  const { timezone, format }: any = nodeData;
  ...
  try {
    ...
    const errorMsg = `タイムゾーン設定エラー: ${error instanceof Error ? error.message : String(error)}`;
    context?.addLog('error', errorMsg, node.id, { timezone, error: error instanceof Error ? error.message : String(error) });
