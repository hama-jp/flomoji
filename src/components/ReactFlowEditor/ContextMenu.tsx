...
const ContextMenu: React.FC = () => {
  ...
  const deleteSelectedElements = useReactFlowStore((state: any) => state.deleteSelectedElements);
  ...
  const renderAddNodeMenu = () => (
    <>
      ...
      {/* Categories */}
      {Object.entries(nodesByCategory).map(([categoryKey, { name, nodes }]: any) => {
          ...
          <div key={categoryKey} className="mb-1">
            ...
            {isExpanded && (
              <div className="ml-2 border-l border-gray-200">
                {Object.entries(nodes).map(([type, config]: any) => (
                  <button
                    key={type}
